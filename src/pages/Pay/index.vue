...
import {reqPayInfo,reqPayStatus} from '@api/pay'
export default {
  name: "Pay",
  data(){
    return{
  ...
  methods:{
    // 使用订单号请求订单信息
    async getPayInfo(){
      const result = await reqPayInfo(this.orderId)
      this.payInfo = result
    },
    // 用来控制对话框的开关
    async pay(){
      ...
      this.timer = setInterval(async () => {
        try{
          await reqPayStatus(this.orderId)
    ...
    async paySuccess(){
      try{
        ...
        this.$message.error('蝙蝠侠说不打你了,快回哥谭吧')
        this.dialogVisible = false
      }
    }
  },
